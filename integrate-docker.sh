...
echo "Exec=$APPPATH --appimage-extract-and-run %u" >> $DESKTOPFILE
...
echo "Exec=$APPPATH --appimage-extract-and-run -new-window" >> $DESKTOPFILE
echo "[Desktop Action new-private-window]" >> $DESKTOPFILE
echo "Name=Open a New Private Window" >> $DESKTOPFILE
echo "Exec=$APPPATH --appimage-extract-and-run -private-window" >> $DESKTOPFILE
